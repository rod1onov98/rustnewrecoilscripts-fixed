...
// M249 animtime	100.0,
// HMLMG animtime	100.0,
// Python animtime	125.0,
// Semi animtime	175.0
...
MouseDriver g_mouseDriver;

bool InitializeMouseDriver()
{
	return g_mouseDriver.Initialize();
}
...
void SlowRec(double wt, double ct, int x1, int y1)
{
	...
	for (int i = 1; i <= (int)ct; ++i)
	{
		...
		if (driverInitialized)
		{
			int deltaX = xI - x_;
			int deltaY = yI - y_;
			g_mouseDriver.MouseMove(deltaX, deltaY);
		}
